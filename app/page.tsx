...
function LocationSearchFields({country,city,onChange,prefix,defaultCountry=false}:{country:string;city:string;onChange:(next:{country:string;city:string})=>void;prefix:string;defaultCountry?:boolean}){
  const countryList=`${prefix}-countries`,cityList=`${prefix}-cities`;
  const matchedCountry=LOCATION_COUNTRIES.find(x=>x===country.trim())||"";
  return <><label>国家 / 地区<input list={countryList} value={country} onChange={e=>onChange({country:e.target.value,city:e.target.value===country?city:""})} placeholder={defaultCountry?"中国":"输入或选择国家 / 地区"}/><datalist id={countryList}>{LOCATION_COUNTRIES.map(x=><option key={x} value={x}/>)}</datalist></label><label>城市<input list={cityList} value={city} onChange={e=>onChange({country,city:e.target.value})} placeholder={matchedCountry?"输入或选择城市":"请先输入或选择国家"} disabled={!matchedCountry}/><datalist id={cityList}>{citiesFor(matchedCountry).map(x=><option key={x.city} value={x.city}/>)}</datalist></label></>;
}
    ...
    try {
      ...
      if (saved) {
        ...
        setData({
          ...
          workoutPlans: (Array.isArray(parsed.workoutPlans) ? parsed.workoutPlans : healthDefaults.workoutPlans).map((plan:WorkoutPlan)=>({...plan,weekStart:plan.weekStart||weekStartKey()})),
