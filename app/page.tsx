...
const FALLBACK_NEWS=["全球重要时事与公共议题","中国经济与消费新观察","人工智能产业最新进展","科技公司与产品动态","金融市场与投资者教育","商业公司与品牌趋势","女性与职场议题","文化出版与新书资讯","国际社会与城市生活","气候、健康与生活方式"].map((title,i)=>({title,source:"今日资讯入口",link:`https://news.google.com/search?q=${encodeURIComponent(title)}&hl=zh-CN&gl=CN&ceid=CN:zh-Hans`,publishedAt:todayKey(),category:["新闻","商业","人工智能","科技","金融"][i%5]}));
...
function Growth({ data, patch, initialTab="path" }: { data: WorkbenchData; patch: (fn:(d:WorkbenchData)=>WorkbenchData)=>void; initialTab?:"path"|"learn"|"goals" }) {
  ...
  const [bookForm,setBookForm]=useState({title:"",author:"",category:"读书",currentPage:"0",totalPages:"0",status:"在读" as ReadingBook["status"]});
  const [podcastForm,setPodcastForm]=useState({title:"",host:"",category:"读书",currentEpisode:"0"});
  const today=todayKey();
  useEffect(()=>{if(tab!=="news")return;const rss=encodeURIComponent("https://news.google.com/rss?hl=zh-CN&gl=CN&ceid=CN:zh-Hans");fetch(`https://api.rss2json.com/v1/api.json?rss_url=${rss}`).then(r=>r.json()).then(j=>{if(Array.isArray(j.items)&&j.items.length)setNews(j.items.slice(0,10).map((x:any)=>({title:x.title,source:x.author||"Google 新闻",link:x.link,publishedAt:x.pubDate||today,category:"今日资讯"}))) }).catch(()=>{})},[tab,today]);
