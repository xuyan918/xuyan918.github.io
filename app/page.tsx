...
function Travel({data,patch}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void}) {
  ...
  const addDestination=async(e:FormEvent)=>{e.preventDefault();if(!destination.place.trim())return;setLocating(true);let position:Partial<Destination>={};try{position=await locatePlace(destination.place,destination.country)}catch{}patch(d=>({...d,destinations:[...d.destinations,{id:uid(),place:destination.place.trim(),visited:false,...position,country:destination.country.trim()||String(position.country||"待补充"),updatedAt:Date.now()}]}));setDestination({place:"",country:""});setLocating(false)};
  const addVisitedCity=async(e:FormEvent)=>{e.preventDefault();if(!visitedCity.place.trim())return;setLocating(true);setMapStatus("正在寻找城市的准确位置…");try{const position=await locatePlace(visitedCity.place,visitedCity.country);patch(d=>({...d,destinations:[...d.destinations,{id:uid(),place:visitedCity.place.trim(),visited:true,...position,updatedAt:Date.now()}]}));setVisitedCity({place:"",country:""});setMapStatus("城市已经准确点亮在地图上。") }catch{setMapStatus("暂时没有找到这个城市，请补充国家或地区后再试。") }finally{setLocating(false)}};
