...
function Health({ data, patch, initialTab="cycle" }: { data:WorkbenchData; patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void; initialTab?:"cycle"|"fitness"|"food"|"care" }) {
  ...
  return <div className="page health-page">
    ...
    {tab==="care"&&<section>
      <form className="location-picker" onSubmit={saveLocation}><div><span className="eyebrow">YOUR LOCATION</span><h2>所在地</h2><p>可以输入关键词快速查找，也可以展开列表选择。</p></div><label>国家 / 地区<input list="health-country-options" value={locationForm.country} onChange={e=>{const country=e.target.value;setLocationForm({country,city:LOCATION_COUNTRIES.includes(country)?citiesFor(country)[0].city:""})}} placeholder="输入或选择国家"/><datalist id="health-country-options">{LOCATION_COUNTRIES.map(country=><option key={country} value={country}/>)}</datalist></label><label>城市<input list="health-city-options" value={locationForm.city} onChange={e=>setLocationForm({...locationForm,city:e.target.value})} placeholder="输入或选择城市"/><datalist id="health-city-options">{citiesFor(locationForm.country).map(item=><option key={item.city} value={item.city}/>)}</datalist></label><button disabled={!locationOption(locationForm.country,locationForm.city)||!citiesFor(locationForm.country).some(x=>x.city===locationForm.city)}>应用位置</button></form>
