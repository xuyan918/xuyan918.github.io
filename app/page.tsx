...
function Dashboard({ data, go, goSection, toggleTodo, patch }: { data: WorkbenchData; go: (v: View, d?: string) => void; goSection:(v:"growth"|"health",tab:"learn"|"fitness"|"path")=>void; toggleTodo: (id: string) => void; patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void }) {
  ...
  const checkinMetrics=[
    ...
    {label:"健身",done:fitnessCount,total:1,target:"health" as const,tab:"fitness" as const,detail:fitnessCount?"训练已完成":"等待打卡"},
...
function EventEditor({ item, date, data, patch, close }: { item: EventItem|null; date:string; data:WorkbenchData; patch:any; close:()=>void }) {
  ...
  const submit=(e:FormEvent)=>{e.preventDefault();if(!form.title.trim())return;const title=form.title.trim();const isSport=data.categories.find(c=>c.id===form.categoryId)?.name==="运动";const day=new Date(`${form.date}T12:00:00`).getDay();patch((d:WorkbenchData)=>({...d,events:item?d.events.map(x=>x.id===item.id?{...x,...form,title,updatedAt:Date.now()}:x):[...d.events,{...form,title,id:uid(),updatedAt:Date.now()}],workoutPlans:isSport?d.workoutPlans.map(p=>p.weekday===day?{...p,title,updatedAt:Date.now()}:p):d.workoutPlans}));close();};
