...
const weekStartKey=(date=todayKey())=>{const value=new Date(`${date}T12:00:00`);value.setDate(value.getDate()-((value.getDay()+6)%7));return dayKey(value)};
...
const normalizedLocation=(country:string,city:string)=>findLocationOption(country.trim(),city.trim());
const normalizeWorkoutSchedule=(plans:WorkoutPlan[],events:EventItem[],eventCategories:Category[])=>{
  const currentWeek=weekStartKey(),sportIds=new Set(eventCategories.filter(x=>x.name==="运动"||x.id==="sport").map(x=>x.id));
  const sportEvents=events.filter(x=>sportIds.has(x.categoryId));
  const normalized=plans.map(plan=>{const matching=sportEvents.filter(event=>event.title===plan.title&&new Date(`${event.date}T12:00:00`).getDay()===plan.weekday);const assigned=plan.weekStart||currentWeek;if(!matching.length)return {...plan,weekStart:assigned};if(matching.some(event=>weekStartKey(event.date)===assigned))return {...plan,weekStart:assigned};return {...plan,weekStart:weekStartKey([...matching].sort((a,b)=>b.date.localeCompare(a.date))[0].date)};});
  sportEvents.forEach(event=>{const weekStart=weekStartKey(event.date),weekday=new Date(`${event.date}T12:00:00`).getDay();if(!normalized.some(plan=>plan.weekStart===weekStart&&plan.weekday===weekday&&plan.title===event.title))normalized.push({id:uid(),weekStart,weekday,title:event.title,intensity:"适中",order:normalized.filter(plan=>plan.weekStart===weekStart&&plan.weekday===weekday).length,completedDates:[],updatedAt:event.updatedAt||Date.now()})});
  return normalized;
};
function LocationSearchFields({country,city,onChange,prefix,defaultCountry=false}:{country:string;city:string;onChange:(next:{country:string;city:string})=>void;prefix:string;defaultCountry?:boolean}){
  const [open,setOpen]=useState<"country"|"city"|null>(null);
  const matchedCountry=LOCATION_COUNTRIES.find(x=>x===country.trim())||"";
  const countryOptions=LOCATION_COUNTRIES.filter(x=>!country.trim()||x.toLowerCase().includes(country.trim().toLowerCase()));
  const cityOptions=citiesFor(matchedCountry).filter(x=>!city.trim()||x.city.toLowerCase().includes(city.trim().toLowerCase()));
  const closeOutside=(e:React.FocusEvent<HTMLDivElement>)=>{if(!e.currentTarget.contains(e.relatedTarget as Node))setOpen(null)};
  return <><div className="location-field" onBlur={closeOutside}><span>国家 / 地区</span><div className="location-combo"><input id={`${prefix}-country`} value={country} onFocus={()=>setOpen("country")} onChange={e=>{onChange({country:e.target.value,city:""});setOpen("country")}} placeholder={defaultCountry?"中国":"输入或选择国家 / 地区"} autoComplete="off" aria-expanded={open==="country"}/><button type="button" aria-label="展开国家或地区列表" onClick={()=>setOpen(open==="country"?null:"country")}>⌄</button></div>{open==="country"&&<div className="location-options" role="listbox">{countryOptions.map(x=><button type="button" role="option" aria-selected={x===country} key={x} onClick={()=>{onChange({country:x,city:""});setOpen(null)}}>{x}</button>)}{!countryOptions.length&&<small>没有匹配的国家或地区</small>}</div>}</div><div className="location-field" onBlur={closeOutside}><span>城市</span><div className="location-combo"><input id={`${prefix}-city`} value={city} onFocus={()=>matchedCountry&&setOpen("city")} onChange={e=>{onChange({country,city:e.target.value});setOpen("city")}} placeholder={matchedCountry?"输入或选择城市":"请先选择国家"} disabled={!matchedCountry} autoComplete="off" aria-expanded={open==="city"}/><button type="button" aria-label="展开城市列表" disabled={!matchedCountry} onClick={()=>matchedCountry&&setOpen(open==="city"?null:"city")}>⌄</button></div>{open==="city"&&<div className="location-options" role="listbox">{cityOptions.map(x=><button type="button" role="option" aria-selected={x.city===city} key={x.city} onClick={()=>{onChange({country,city:x.city});setOpen(null)}}><b>{x.city}</b><small>{x.admin1}</small></button>)}{!cityOptions.length&&<small>没有匹配的城市</small>}</div>}</div></>;
}
...
type CalendarMark={name:string;kind:"holiday"|"term"};
type OfficialHolidayDay={name:string;date:string;isOffDay:boolean};
const officialHolidayCacheKey=(year:number)=>`bear-workbench-cn-holidays-${year}`;
const validOfficialHolidayDays=(value:any):OfficialHolidayDay[]=>Array.isArray(value?.days)?value.days.filter((x:any)=>typeof x?.name==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(x?.date)&&typeof x?.isOffDay==="boolean"):[];
const fetchOfficialHolidayYear=async(year:number)=>{const urls=[`https://raw.githubusercontent.com/NateScarlet/holiday-cn/master/${year}.json`,`https://cdn.jsdelivr.net/gh/NateScarlet/holiday-cn@master/${year}.json`];for(const url of urls){try{const response=await fetch(url,{cache:"no-cache"});if(!response.ok)continue;const data=await response.json(),days=validOfficialHolidayDays(data);if(days.length)return days}catch{}}return [] as OfficialHolidayDay[]};
const solarTermForDate=(date:string)=>{const d=new Date(`${date}T12:00:00`),year=d.getFullYear(),y=year%100,index=d.getMonth()*2+(d.getDate()>15?1:0);const terms=[["小寒",5.4055],["大寒",20.12],["立春",3.87],["雨水",18.73],["惊蛰",5.63],["春分",20.646],["清明",4.81],["谷雨",20.1],["立夏",5.52],["小满",21.04],["芒种",5.678],["夏至",21.37],["小暑",7.108],["大暑",22.83],["立秋",7.5],["处暑",23.13],["白露",7.646],["秋分",23.042],["寒露",8.318],["霜降",23.438],["立冬",7.438],["小雪",22.36],["大雪",7.18],["冬至",21.94]] as const;const [name,c]=terms[index];const leapBase=d.getMonth()<2?Math.floor((y-1)/4):Math.floor(y/4);const day=Math.floor(y*.2422+c)-leapBase;return d.getDate()===day?name:""};
const calendarMarksForDate=(date:string):CalendarMark[]=>{const d=new Date(`${date}T12:00:00`),month=d.getMonth()+1,day=d.getDate(),key=`${month}-${day}`,marks:CalendarMark[]=[];const fixed:Record<string,string[]>={"1-1":["元旦"],"2-14":["情人节"],"3-8":["国际妇女节"],"3-12":["植树节"],"4-1":["愚人节"],"5-1":["国际劳动节"],"5-4":["青年节"],"6-1":["国际儿童节"],"8-1":["建军节"],"9-10":["教师节"],"10-1":["国庆节"],"10-31":["万圣节"],"12-24":["平安夜"],"12-25":["圣诞节"]};(fixed[key]||[]).forEach(name=>marks.push({name,kind:"holiday"}));if(month===5&&d.getDay()===0&&day>=8&&day<=14)marks.push({name:"母亲节",kind:"holiday"});if(month===6&&d.getDay()===0&&day>=15&&day<=21)marks.push({name:"父亲节",kind:"holiday"});if(month===11&&d.getDay()===4&&day>=22&&day<=28)marks.push({name:"感恩节",kind:"holiday"});const lunar=lunarParts(date),lunarFestivals:Record<string,string>={"1-1":"春节","1-15":"元宵节","2-2":"龙抬头","5-5":"端午节","7-7":"七夕节","7-15":"中元节","8-15":"中秋节","9-9":"重阳节","12-8":"腊八节","12-23":"北方小年","12-24":"南方小年"};const lunarName=lunarFestivals[`${lunar.month}-${lunar.day}`];if(lunarName)marks.push({name:lunarName,kind:"holiday"});const tomorrow=lunarParts(addDays(date,1));if(lunar.month===12&&tomorrow.month===1&&tomorrow.day===1)marks.push({name:"除夕",kind:"holiday"});const term=solarTermForDate(date);if(term)marks.push({name:term,kind:"term"});return marks.filter((mark,index,list)=>list.findIndex(x=>x.name===mark.name)===index)};
...
const specialDaysForDate=(items:SpecialDay[],date:string)=>[...items.filter(item=>occursOn(item,date)).map(item=>item.id.startsWith("promo-")||item.kind==="生日"||item.kind==="纪念日"||/^[㊡㊤]/.test(item.title)?item:item.kind==="节日"?{...item,title:`❖ ${item.title}`}:item.kind==="演唱会"?{...item,title:`♫ ${item.title}`}:item),...calendarMarksForDate(date).map((mark,index):SpecialDay=>({id:`built-in-${mark.kind}-${date}-${index}`,title:`${mark.kind==="term"?"◇":"🎈"} ${mark.name}`,date,kind:mark.kind==="term"?"节气":"默认节日",calendar:"公历",reminderDays:0,updatedAt:-1000+index}))].sort((a,b)=>specialRank(a)-specialRank(b)||a.updatedAt-b.updatedAt);
...
const phaseThreeDefaults = (now = Date.now()) => {
  ...
  return {
    ...
    workoutPlans: [
      {id:"w1",weekStart:weekStartKey(),weekday:1,title:"普拉提 · 核心稳定",intensity:"适中" as const,updatedAt:now},
      {id:"w2",weekStart:weekStartKey(),weekday:2,title:"快走 30 分钟",intensity:"适中" as const,updatedAt:now},
      {id:"w3",weekStart:weekStartKey(),weekday:3,title:"普拉提 · 全身塑形",intensity:"适中" as const,updatedAt:now},
      {id:"w4",weekStart:weekStartKey(),weekday:4,title:"拉伸与肩颈放松",intensity:"轻柔" as const,updatedAt:now},
      {id:"w5",weekStart:weekStartKey(),weekday:5,title:"普拉提 · 下肢力量",intensity:"适中" as const,updatedAt:now},
      {id:"w6",weekStart:weekStartKey(),weekday:6,title:"户外散步或轻徒步",intensity:"适中" as const,updatedAt:now},
      {id:"w0",weekStart:weekStartKey(),weekday:0,title:"舒缓拉伸与恢复",intensity:"轻柔" as const,updatedAt:now},
    ],
...
const greetings = [
  ...
  "不慌不忙地生活，也是一种很了不起的能力。",
  "把注意力放回自己，今天会有新的小收获。",
  "愿今天的风景里，有一件让你嘴角上扬的小事。",
  "允许计划留一点空白，生活会悄悄送来新的惊喜。",
  "认真吃饭、好好睡觉，也是照顾未来的自己。",
  "今天的你不需要完美，真实而舒展就很好。",
  "把复杂的事情一件件做好，心也会慢慢安定下来。",
  "累的时候先停一停，休息从来不是落后。",
  "每一次温柔而坚定的选择，都在靠近喜欢的生活。",
  "愿你拥有专注做事的清醒，也有自在发呆的松弛。",
  "不用急着看见答案，认真走过的路自有意义。",
  "给自己一点肯定，你已经完成了许多不容易的事。",
  "愿今天有好消息，也有被小小幸福接住的时刻。",
  "慢一点没关系，稳定地向前就是很好的进度。",
];
...
function Calendar({ data, dates, selectedDate, setSelectedDate, category, toggleTodo, move, onAdd, onEdit, onDelete, patch }: any) {
  ...
  const [categoryDraft,setCategoryDraft]=useState({name:"",color:"#D88996"});
  const requestedHolidayYears=useRef(new Set<number>());
  ...
  useEffect(()=>{
    const frame=requestAnimationFrame(()=>{
      document.querySelectorAll<HTMLElement>(".calendar-page .calendar-checkins b").forEach(node=>{
        const label=node.textContent||"";
        if(/^✦\s+(?:[◇❖♫㊡㊤]|🎈)/u.test(label))node.textContent=label.replace(/^✦\s+/,"");
      });
    });
    return()=>cancelAnimationFrame(frame);
  },[weekDates[0],weekDates[6],data.specialDays]);
  useEffect(()=>{const years=[...new Set(weekDates.map(date=>Number(date.slice(0,4))))];const sync=(year:number,days:OfficialHolidayDay[])=>{if(!days.length)return;patch((d:WorkbenchData)=>{const prefix=`official-cn-${year}-`,next=days.map((item,index):SpecialDay=>({id:`${prefix}${item.date}`,title:`${item.isOffDay?"㊡":"㊤"} ${item.name}`,date:item.date,kind:"法定节假日",calendar:"公历",reminderDays:0,updatedAt:-2000+index}));const current=d.specialDays.filter(x=>x.id.startsWith(prefix));const signature=(list:SpecialDay[])=>list.map(x=>`${x.id}:${x.title}`).sort().join("|");return signature(current)===signature(next)?d:{...d,specialDays:[...d.specialDays.filter(x=>!x.id.startsWith(prefix)),...next]}})};years.forEach(year=>{if(requestedHolidayYears.current.has(year))return;requestedHolidayYears.current.add(year);try{const cached=JSON.parse(localStorage.getItem(officialHolidayCacheKey(year))||"null"),days=validOfficialHolidayDays(cached);sync(year,days)}catch{}fetchOfficialHolidayYear(year).then(days=>{if(!days.length)return;try{localStorage.setItem(officialHolidayCacheKey(year),JSON.stringify({year,days,updatedAt:Date.now()}))}catch{}sync(year,days)})})},[weekDates[0],weekDates[6],patch]);
  ...
  const parseQuick = () => {
    ...
    const categoryName=/普拉提|瑜伽|跑步|健身|训练|游泳|骑行|羽毛球|网球|运动/.test(title)?"运动":/电影|游戏|演唱会|音乐会|看展/.test(title)?"娱乐":/购物|买菜|家务|吃饭|取快递|收快递/.test(title)?"生活":/工作|会议|面试|复盘|RPA|汇报/.test(title)?"工作":"其他";
    const categoryId=data.categories.find((x:Category)=>x.name===categoryName)?.id||data.categories[0]?.id;
    if(!categoryId)return;
    const endD = new Date(`2000-01-01T${start}:00`); endD.setHours(endD.getHours()+1);
    patch((d:WorkbenchData)=>{const event={id:uid(),date,start,end:`${pad(endD.getHours())}:${pad(endD.getMinutes())}`,title,categoryId,updatedAt:Date.now()};const isSport=d.categories.find(x=>x.id===categoryId)?.name==="运动",weekStart=weekStartKey(date),weekday=new Date(`${date}T12:00:00`).getDay();const exists=d.workoutPlans.some(p=>(p.weekStart||weekStartKey())===weekStart&&p.weekday===weekday&&p.title===title);return {...d,events:[...d.events,event],workoutPlans:isSport&&!exists?[...d.workoutPlans,{id:uid(),weekStart,weekday,title,intensity:"适中",order:d.workoutPlans.filter(p=>(p.weekStart||weekStartKey())===weekStart&&p.weekday===weekday).length,completedDates:[],updatedAt:Date.now()}]:d.workoutPlans}});setQuickText("");setSelectedDate(date);
  };
  return <div className="page calendar-page">
    <header className="page-head"><div><span className="eyebrow">MY WEEK</span><h1>本周日程与待办</h1></div><div className="calendar-head-actions"><button className="secondary" onClick={()=>setShowCategories(true)}>管理分类</button><button onClick={onAdd}>＋ 新建日程</button></div></header>
    ...
    {showCategories&&<Modal title="管理日程分类" onClose={()=>setShowCategories(false)}><form className="calendar-category-add" onSubmit={e=>{e.preventDefault();const name=categoryDraft.name.trim();if(!name||data.categories.some((x:Category)=>x.name===name))return;patch((d:WorkbenchData)=>({...d,categories:[...d.categories,{id:uid(),name,color:categoryDraft.color}]}));setCategoryDraft({...categoryDraft,name:""})}}><label>分类颜色<input type="color" value={categoryDraft.color} onChange={e=>setCategoryDraft({...categoryDraft,color:e.target.value})}/></label><label>新分类名称<input value={categoryDraft.name} onChange={e=>setCategoryDraft({...categoryDraft,name:e.target.value})} placeholder="例如：约会、家人、宠物" required/></label><button>＋ 新增分类</button></form><p className="category-manager-note">修改会立即同步到已有日程；删除分类后，其中的日程会自动转入其他可用分类。</p><div className="calendar-category-list">{data.categories.map((item:Category)=><article key={item.id}><input type="color" aria-label={`${item.name}颜色`} value={item.color} onChange={e=>patch((d:WorkbenchData)=>({...d,categories:d.categories.map(x=>x.id===item.id?{...x,color:e.target.value}:x)}))}/><input aria-label="分类名称" value={item.name} onChange={e=>patch((d:WorkbenchData)=>({...d,categories:d.categories.map(x=>x.id===item.id?{...x,name:e.target.value}:x)}))}/><span>{data.events.filter((x:EventItem)=>x.categoryId===item.id).length} 项日程</span><button disabled={data.categories.length<=1} onClick={()=>patch((d:WorkbenchData)=>{const fallback=d.categories.find(x=>x.id!==item.id);return fallback?{...d,categories:d.categories.filter(x=>x.id!==item.id),events:d.events.map(x=>x.categoryId===item.id?{...x,categoryId:fallback.id,updatedAt:Date.now()}:x)}:d})}>删除</button></article>)}</div></Modal>}
  </div>;
}
...
function Memos({ data, move, onAdd, onDelete, patch }: any) {
  ...
  const moodSticker=(entry:MoodEntry)=>{const tags=entry.tagIds.map(id=>data.moodTags.find((x:MoodTag)=>x.id===id)?.name||"").join(" ");if(/美食/.test(tags))return "🍰";if(/阅读/.test(tags))return "📖";if(/朋友|社交/.test(tags))return "🎀";if(/工作/.test(tags))return "✎";if(/旅行/.test(tags))return "✈";if(/猫|宠物/.test(tags))return "🐾";return ({开心:"🌷",平静:"☁️",期待:"✨",疲惫:"🌙",低落:"🌧️",生气:"💭",焦虑:"🫧"} as Record<string,string>)[entry.mood]||"♡"};
...
function Health({ data, patch, initialTab="cycle" }: { data:WorkbenchData; patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void; initialTab?:"cycle"|"fitness"|"food"|"care" }) {
  ...
  const mondayOf=(date:string)=>weekStartKey(date);
  const currentFitnessWeek=weekStartKey();
  ...
  const weekPlans=data.workoutPlans.filter(p=>(p.weekStart||currentFitnessWeek)===fitnessWeek);
  const workoutTemplatePools:[string,WorkoutPlan["intensity"]][][]=[
    [["普拉提 · 核心唤醒","适中"],["快走与髋部拉伸","适中"],["上肢弹力带训练","适中"],["肩颈瑜伽与呼吸","轻柔"],["臀腿力量循环","较高"],["户外慢跑或骑行","适中"],["全身筋膜放松","轻柔"]],
    [["低冲击有氧 25 分钟","适中"],["普拉提 · 下肢塑形","适中"],["背部与手臂力量","较高"],["散步 40 分钟","轻柔"],["核心稳定与平衡","适中"],["舞蹈有氧或游泳","较高"],["睡前舒缓拉伸","轻柔"]],
    [["瑜伽流动与唤醒","轻柔"],["间歇快走 30 分钟","较高"],["普拉提 · 全身控制","适中"],["泡沫轴恢复","轻柔"],["深蹲与臀桥组合","较高"],["轻徒步或羽毛球","适中"],["冥想呼吸与拉伸","轻柔"]],
    [["核心耐力小循环","适中"],["胸背姿态训练","适中"],["低冲击燃脂操","较高"],["髋踝灵活性练习","轻柔"],["普拉提 · 侧腰与腿部","适中"],["公园快走 45 分钟","适中"],["休息日轻拉伸","轻柔"]],
  ];
  const createWeekPlans=(week:string)=>{const pool=workoutTemplatePools[dailyIndex(week,workoutTemplatePools.length)],now=Date.now();return pool.map(([title,intensity],index)=>({id:uid(),weekStart:week,weekday:[1,2,3,4,5,6,0][index],title,intensity,order:0,completedDates:[],updatedAt:now}))};
  const addRecommendedWeek=()=>{if(weekPlans.length)return;patch(d=>({...d,workoutPlans:[...d.workoutPlans,...createWeekPlans(fitnessWeek)]}))};
  const openFitnessWeek=(next:string)=>{setFitnessWeek(next);if(next>currentFitnessWeek&&!data.workoutPlans.some(p=>(p.weekStart||currentFitnessWeek)===next))patch(d=>({...d,workoutPlans:[...d.workoutPlans,...createWeekPlans(next)]}))};
  ...
  const addWorkout=(e:FormEvent)=>{e.preventDefault();if(!workoutForm.title.trim())return;patch(d=>({...d,workoutPlans:[...d.workoutPlans,{id:uid(),weekStart:fitnessWeek,weekday:workoutForm.weekday,title:workoutForm.title.trim(),intensity:workoutForm.intensity,order:d.workoutPlans.filter(p=>(p.weekStart||currentFitnessWeek)===fitnessWeek&&p.weekday===workoutForm.weekday).length,completedDates:[],updatedAt:Date.now()}]}));setWorkoutForm({...workoutForm,title:""});setShowWorkoutAdd(false)};
  const moveWorkout=(id:string,direction:number)=>patch(d=>{const current=d.workoutPlans.find(p=>p.id===id);if(!current)return d;const group=d.workoutPlans.filter(p=>(p.weekStart||currentFitnessWeek)===(current.weekStart||currentFitnessWeek)&&p.weekday===current.weekday).sort((a,b)=>(a.order??0)-(b.order??0));const index=group.findIndex(p=>p.id===id),target=index+direction;if(target<0||target>=group.length)return d;[group[index],group[target]]=[group[target],group[index]];const orders=new Map(group.map((p,i)=>[p.id,i]));return {...d,workoutPlans:d.workoutPlans.map(p=>orders.has(p.id)?{...p,order:orders.get(p.id),updatedAt:Date.now()}:p)}});
  ...
  const saveLocation=(e:FormEvent)=>{e.preventDefault();const selected=normalizedLocation(locationForm.country,locationForm.city);if(!selected)return;patch(d=>({...d,healthSettings:{...d.healthSettings,...selected,updatedAt:Date.now()}}));setLocationForm({country:selected.country,city:selected.city})};
  ...
  return <div className="page health-page">
    ...
    {tab==="fitness"&&<section>
      ...
      <nav className="fitness-history-nav" aria-label="健身计划周选择"><button className="week-prev" onClick={()=>openFitnessWeek(addDays(fitnessWeek,-7))}>← 上一周</button><label>选择日期<input type="date" value={fitnessWeek} onChange={e=>openFitnessWeek(mondayOf(e.target.value))}/></label><b>{displayDate(fitnessWeek)} — {displayDate(fitnessWeekEnd)}</b><button className="week-current" onClick={()=>openFitnessWeek(currentFitnessWeek)} disabled={selectedCurrentWeek}>回到本周</button><button className="week-next" onClick={()=>openFitnessWeek(addDays(fitnessWeek,7))}>下一周 →</button></nav>
      {!weekPlans.length&&<div className="workout-week-empty"><span>这一周还没有安排运动，可以从空白开始，也可以换一组不重复的推荐。</span><button onClick={addRecommendedWeek}>✦ 生成本周推荐</button></div>}
      {selectedCurrentWeek&&<aside className="today-workout wide"><span className="eyebrow">TODAY</span><h2>今天的运动</h2><p>{advice.sport}</p><div className="today-workout-list">{weekPlans.filter(p=>p.weekday===new Date().getDay()).sort((a,b)=>(a.order??0)-(b.order??0)).map(plan=><label key={plan.id}><input type="checkbox" checked={plan.completedDates?.includes(today)||false} onChange={()=>toggleWorkout(plan.id,today)}/><i></i><span><b className={plan.completedDates?.includes(today)?"strike":""}>{plan.title}</b><small>{plan.intensity}</small></span></label>)}</div><strong>{weekPlans.filter(p=>p.weekday===new Date().getDay()&&p.completedDates?.includes(today)).length} / {weekPlans.filter(p=>p.weekday===new Date().getDay()).length} 项完成</strong></aside>}
      <section className="week-plan multi-day">{[1,2,3,4,5,6,0].map(day=>{const date=workoutDate(day);const plans=weekPlans.filter(p=>p.weekday===day).sort((a,b)=>(a.order??0)-(b.order??0));return <article key={date} className={date===today?"today":""}><header><span>{weekdays[day]}<small>{displayDate(date)}</small></span>{date===today&&<b>今天</b>}<button onClick={()=>{setWorkoutForm({...workoutForm,weekday:day,title:""});setShowWorkoutAdd(true)}}>＋ 添加</button></header><div className="workout-items">{plans.map(plan=><div className="workout-item" key={plan.id}><label><input type="checkbox" checked={plan.completedDates?.includes(date)||false} onChange={()=>toggleWorkout(plan.id,date)}/><i></i></label><div><input className={plan.completedDates?.includes(date)?"strike":""} value={plan.title} onChange={e=>patch(d=>({...d,workoutPlans:d.workoutPlans.map(p=>p.id===plan.id?{...p,title:e.target.value,updatedAt:Date.now()}:p)}))}/><select value={plan.intensity} onChange={e=>patch(d=>({...d,workoutPlans:d.workoutPlans.map(p=>p.id===plan.id?{...p,intensity:e.target.value as WorkoutPlan["intensity"],updatedAt:Date.now()}:p)}))}><option>轻柔</option><option>适中</option><option>较高</option></select></div><span><button onClick={()=>moveWorkout(plan.id,-1)} aria-label="上移">↑</button><button onClick={()=>moveWorkout(plan.id,1)} aria-label="下移">↓</button><button onClick={()=>setDeleteWorkout(plan.id)} aria-label="删除">×</button></span></div>)}{!plans.length&&<p className="workout-empty">这一天没有安排运动。</p>}</div></article>})}</section>
    ...
    {tab==="care"&&<section>
      <form className="location-picker" onSubmit={saveLocation}><div><span className="eyebrow">YOUR LOCATION</span><h2>所在地</h2><p>可以直接输入搜索，也可以展开候选列表选择。</p></div><LocationSearchFields prefix="care-location" country={locationForm.country} city={locationForm.city} onChange={setLocationForm} defaultCountry/><button disabled={!normalizedLocation(locationForm.country,locationForm.city)}>应用位置</button></form>
...
function TrendCanvas({ points }:{ points:{label:string;income:number;expense:number}[] }) {
  ...
  useEffect(()=>{
    ...
    const max=Math.max(1,...points.map(p=>p.expense));const padX=20,padY=18,chartW=width-padX*2,chartH=height-padY*2;
    const draw=(color:string)=>{ctx.beginPath();points.forEach((p,i)=>{const x=padX+(points.length===1?0:i/(points.length-1))*chartW;const y=padY+chartH-(p.expense/max)*chartH;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.strokeStyle=color;ctx.lineWidth=2.2;ctx.lineCap="round";ctx.lineJoin="round";ctx.stroke();points.forEach((p,i)=>{const x=padX+(points.length===1?0:i/(points.length-1))*chartW;const y=padY+chartH-(p.expense/max)*chartH;ctx.beginPath();ctx.arc(x,y,2.8,0,Math.PI*2);ctx.fillStyle=color;ctx.fill()})};
...
function EventEditor({ item, date, data, patch, close }: { item: EventItem|null; date:string; data:WorkbenchData; patch:any; close:()=>void }) {
  const [form,setForm]=useState({date:item?.date||date,start:item?.start||"09:00",end:item?.end||item?.start||"09:00",title:item?.title||"",categoryId:item?.categoryId||data.categories[0]?.id||""});
  const submit=(e:FormEvent)=>{e.preventDefault();if(!form.title.trim()||!form.categoryId)return;const title=form.title.trim();const isSport=data.categories.find(c=>c.id===form.categoryId)?.name==="运动";const day=new Date(`${form.date}T12:00:00`).getDay(),targetWeek=weekStartKey(form.date);patch((d:WorkbenchData)=>{let workoutPlans=d.workoutPlans;if(isSport&&item){const oldDay=new Date(`${item.date}T12:00:00`).getDay(),oldWeek=weekStartKey(item.date);let changed=false;workoutPlans=d.workoutPlans.map(p=>{if(!changed&&(p.weekStart||weekStartKey())===oldWeek&&p.weekday===oldDay&&p.title===item.title){changed=true;return {...p,weekStart:targetWeek,weekday:day,title,updatedAt:Date.now()}}return p});if(!changed)workoutPlans=[...workoutPlans,{id:uid(),weekStart:targetWeek,weekday:day,title,intensity:"适中",order:workoutPlans.filter(p=>(p.weekStart||weekStartKey())===targetWeek&&p.weekday===day).length,completedDates:[],updatedAt:Date.now()}]}else if(isSport)workoutPlans=[...d.workoutPlans,{id:uid(),weekStart:targetWeek,weekday:day,title,intensity:"适中",order:d.workoutPlans.filter(p=>(p.weekStart||weekStartKey())===targetWeek&&p.weekday===day).length,completedDates:[],updatedAt:Date.now()}];return {...d,events:item?d.events.map(x=>x.id===item.id?{...x,...form,title,updatedAt:Date.now()}:x):[...d.events,{...form,title,id:uid(),updatedAt:Date.now()}],workoutPlans}});close();};
...
function Jobs({data,patch,embedded=false}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void;embedded?:boolean}) {
  ...
  const criterionPlaceholder=(kind:JobCriterion["kind"])=>kind==="关键词"?"填写希望匹配的岗位方向":kind==="地点"?"填写希望工作的城市或区域":"填写可接受的公司规模下限";
  const sourceKeywords=data.jobCriteria.filter(c=>c.kind==="关键词").map(c=>c.value.trim()).filter(Boolean).join(" ");
  const sourceLocations=data.jobCriteria.filter(c=>c.kind==="地点").map(c=>c.value.trim()).filter(Boolean).join(" ");
  ...
  return <div className={`jobs-page ${embedded?"embedded":""}`}>
    ...
    <section className="job-rule">
      ...
      <div className="job-criteria">
        {data.jobCriteria.map(c=><article key={c.id}><select value={c.kind} onChange={e=>updateCriterion(c.id,{kind:e.target.value as JobCriterion["kind"],value:""})}><option>关键词</option><option>地点</option><option>最低公司人数</option></select><input type={c.kind==="最低公司人数"?"number":"text"} min={c.kind==="最低公司人数"?"1":undefined} value={c.value} onChange={e=>updateCriterion(c.id,{value:e.target.value})} placeholder={criterionPlaceholder(c.kind)}/><button onClick={()=>patch(d=>({...d,jobCriteria:d.jobCriteria.filter(x=>x.id!==c.id)}))} aria-label="删除筛选条件">×</button></article>)}
    ...
    <section className="job-grid">
      {filtered.map(job=><article className="job-card" key={job.id}>
        ...
        <div className="job-meta"><b>{job.salary}</b><span>{job.location}</span><span>{job.companySize>=1000?"1000 人以上":`${job.companySize}–499 人`}</span></div>
        <p>{job.jd}</p>
        <div className="match-reasons">{sourceKeywords&&<span>✓ 关键词匹配</span>}{sourceLocations&&<span>✓ 地点匹配</span>}{sourceMinSize&&<span>✓ 规模符合</span>}{!data.jobCriteria.length&&<span>✓ 已保存岗位</span>}</div>
      ...
      {!filtered.length&&<Empty text="本机还没有符合条件的已保存岗位，可以先从上方公开入口查看并录入感兴趣的岗位。"/>}
    </section>
...
function Travel({data,patch}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void}) {
  ...
  const addDestination=(e:FormEvent)=>{e.preventDefault();const selected=normalizedLocation(destination.country,destination.place);if(!selected)return;patch(d=>d.destinations.some(x=>x.country===selected.country&&x.place===selected.city)?d:{...d,destinations:[...d.destinations,{id:uid(),place:selected.city,country:selected.country,visited:false,latitude:selected.latitude,longitude:selected.longitude,updatedAt:Date.now()}]});setDestination({country:"",place:""})};
  const addVisitedCity=(e:FormEvent)=>{e.preventDefault();const selected=normalizedLocation(visitedCity.country,visitedCity.place);if(!selected)return;patch(d=>{const exists=d.destinations.some(x=>x.country===selected.country&&x.place===selected.city);return {...d,destinations:exists?d.destinations.map(x=>x.country===selected.country&&x.place===selected.city?{...x,visited:true,latitude:selected.latitude,longitude:selected.longitude,updatedAt:Date.now()}:x):[...d.destinations,{id:uid(),place:selected.city,country:selected.country,visited:true,latitude:selected.latitude,longitude:selected.longitude,updatedAt:Date.now()}]}});setMapStatus(`${selected.country} · ${selected.city} 已准确点亮在地图上。`);setVisitedCity({country:"",place:""})};
