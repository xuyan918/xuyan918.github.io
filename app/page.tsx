...
type Goal = { id: string; kind: "证书" | "技能"; text: string; done: boolean; updatedAt: number };
...
const LOCATION_OPTIONS:CityOption[]=[
  ...
  ["美国","纽约","纽约州",40.7128,-74.006,"America/New_York"],["美国","洛杉矶","加利福尼亚州",34.0522,-118.2437,"America/Los_Angeles"],["美国","旧金山","加利福尼亚州",37.7749,-122.4194,"America/Los_Angeles"],["加拿大","温哥华","不列颠哥伦比亚省",49.2827,-123.1207,"America/Vancouver"],["澳大利亚","悉尼","新南威尔士州",-33.8688,151.2093,"Australia/Sydney"],["新西兰","奥克兰","奥克兰",-36.8509,174.7645,"Pacific/Auckland"],["阿联酋","迪拜","迪拜",25.2048,55.2708,"Asia/Dubai"]
...
const locationOption=(country:string,city:string)=>LOCATION_OPTIONS.find(x=>x.country===country&&(x.city===city||`${x.city}市`===city))||LOCATION_OPTIONS[0];
...
function GeoWorldMap({places}:{places:Destination[]}){
  ...
  return <div className="geo-world-map">
    <svg ref={svgRef} viewBox={`${clampedCenter.x-viewWidth/2} ${clampedCenter.y-viewHeight/2} ${viewWidth} ${viewHeight}`} aria-label="可缩放的世界地图" onWheel={e=>{e.preventDefault();changeZoom(zoom*(e.deltaY<0?1.25:.8))}} onPointerDown={e=>{drag.current={x:e.clientX,y:e.clientY,centerX:center.x,centerY:center.y};e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(!drag.current||!svgRef.current)return;const rect=svgRef.current.getBoundingClientRect();setCenter({x:drag.current.centerX-(e.clientX-drag.current.x)*viewWidth/rect.width,y:drag.current.centerY-(e.clientY-drag.current.y)*viewHeight/rect.height})}} onPointerUp={()=>{drag.current=null}} onPointerCancel={()=>{drag.current=null}}>
      ...
      <g className="map-markers">{places.filter(x=>Number.isFinite(x.latitude)&&Number.isFinite(x.longitude)).map(place=>{const point=mapPoint(place.longitude!,place.latitude!);return <g key={place.id} transform={`translate(${point.x} ${point.y})`}><circle r={5/Math.sqrt(zoom)}/><circle className="marker-core" r={2/Math.sqrt(zoom)}/>{zoom>=2&&<text x={8/zoom} y={-7/zoom} fontSize={11/zoom}>{place.place}</text>}<title>{place.place} · {place.country}</title></g>})}</g>
    </svg>
    ...
    <small>双指或滚轮缩放 · 拖动查看 · 红点为准确经纬度位置</small>
  </div>;
}
...
function Health({ data, patch, initialTab="cycle" }: { data:WorkbenchData; patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void; initialTab?:"cycle"|"fitness"|"food"|"care" }) {
  ...
  return <div className="page health-page">
    ...
    {tab==="care"&&<section>
      <form className="location-picker" onSubmit={saveLocation}><div><span className="eyebrow">YOUR LOCATION</span><h2>所在地</h2><p>天气、气候、时令与相关建议会跟随这里更新。</p></div><label>国家 / 地区<select value={locationForm.country} onChange={e=>{const country=e.target.value;setLocationForm({country,city:citiesFor(country)[0].city})}}>{LOCATION_COUNTRIES.map(country=><option key={country}>{country}</option>)}</select></label><label>城市<select value={locationForm.city} onChange={e=>setLocationForm({...locationForm,city:e.target.value})}>{citiesFor(locationForm.country).map(item=><option key={item.city}>{item.city}</option>)}</select></label><button>应用位置</button></form>
...
function Travel({data,patch}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void}) {
  ...
  const [destination,setDestination]=useState({place:"杭州",country:"中国"});
  const [visitedCity,setVisitedCity]=useState({place:"杭州",country:"中国"});
  ...
  useEffect(()=>{const missing=data.destinations.find(x=>x.visited&&(!Number.isFinite(x.latitude)||!Number.isFinite(x.longitude)));if(!missing)return;let active=true;locatePlace(missing.place,missing.country).then(position=>{if(active)patch(d=>({...d,destinations:d.destinations.map(x=>x.id===missing.id?{...x,...position,updatedAt:Date.now()}:x)}))}).catch(()=>{});return()=>{active=false}},[data.destinations,patch]);
  const addDestination=(e:FormEvent)=>{e.preventDefault();const selected=locationOption(destination.country,destination.place);patch(d=>({...d,destinations:[...d.destinations,{id:uid(),place:selected.city,country:selected.country,visited:false,latitude:selected.latitude,longitude:selected.longitude,updatedAt:Date.now()}]}))};
  const addVisitedCity=(e:FormEvent)=>{e.preventDefault();const selected=locationOption(visitedCity.country,visitedCity.place);patch(d=>({...d,destinations:[...d.destinations,{id:uid(),place:selected.city,country:selected.country,visited:true,latitude:selected.latitude,longitude:selected.longitude,updatedAt:Date.now()}]}));setMapStatus(`${selected.country} · ${selected.city} 已准确点亮在地图上。`)};
