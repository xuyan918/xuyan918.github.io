...
function Dashboard({ data, go, goSection, toggleTodo, patch }: { data: WorkbenchData; go: (v: View, d?: string) => void; goSection:(v:"growth"|"health",tab:"learn"|"fitness"|"path")=>void; toggleTodo: (id: string) => void; patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void }) {
  ...
  return <div className="page dashboard">
    ...
    {reminders.length>0&&<button className="dashboard-reminder" onClick={()=>go("travel")}><span>♡ 未来三天提醒</span><div>{reminders.map(x=><b key={x.item.id}>{x.item.kind==="生日"?"🎂":"✦"} {x.item.title}<small>{x.date===today?"今天":`${dateDiff(today,x.date)} 天后`}</small></b>)}{allReminders.length>reminders.length&&<b>另有 {allReminders.length-reminders.length} 项优惠</b>}</div><i>去看看 →</i></button>}
...
function Travel({data,patch}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void}) {
  ...
  return <div className="page travel-page">
    ...
    {tab==="dates"&&<section><div className="travel-section-head"><div><span className="eyebrow">SPECIAL DAYS</span><h2>值得期待的日子，不要错过</h2><p>会员日会按每周、每月或每年自动重复。</p></div><button onClick={()=>setShowSpecial(true)}>＋ 添加特别日子</button></div><div className="special-groups">{["weekly","monthly","yearly","once"].map(group=>{const items=data.specialDays.filter(x=>group==="once"?!x.recurrence:x.recurrence===group).sort((a,b)=>nextSpecialDate(a).localeCompare(nextSpecialDate(b)));if(!items.length)return null;return <section key={group}><h3>{group==="weekly"?"每周会员日":group==="monthly"?"每月会员日":group==="yearly"?"每年生日权益":"单次特别日子"} <span>{items.length}</span></h3><div className="special-grid">{items.map(x=>{const next=nextSpecialDate(x);return <article key={x.id}><i>{x.kind==="生日"?"🎂":x.kind==="演唱会"?"♫":x.kind==="折扣"?"％":"✦"}</i><div><span>{specialRuleLabel(x)} · 下次 {displayDate(next)}</span><h3>{x.title}</h3><p>{x.recurrence?"到期自动进入下一轮":`提前 ${x.reminderDays} 天提醒`}</p></div><button onClick={()=>setDeleteTarget({kind:"special",id:x.id})}>×</button></article>})}</div></section>})}</div></section>}
