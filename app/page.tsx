...
function Calendar({ data, dates, selectedDate, setSelectedDate, category, toggleTodo, move, onAdd, onEdit, onDelete, patch }: any) {
  ...
  return <div className="page calendar-page">
    <header className="page-head"><div><span className="eyebrow">MY WEEK</span><h1>本周日程与待办</h1><p>七天同时展开，方便协调每天的时间与任务。</p></div><button onClick={onAdd}>＋ 新建日程</button></header>
...
function Memos({ data, go, toggleTodo, move, onAdd, onDelete, patch }: any) {
  ...
  return <div className="page memo-page"><header className="page-head"><div><span className="eyebrow">LITTLE NOTES</span><h1>备忘</h1><p>所有历史灵感都收在这里，最新添加的排在最前面。</p></div><button onClick={onAdd}>＋ 快速备忘</button></header>
...
function Travel({data,patch}:{data:WorkbenchData;patch:(fn:(d:WorkbenchData)=>WorkbenchData)=>void}) {
  ...
  return <div className="page travel-page">
    ...
    {tab==="dates"&&<section><div className="travel-section-head"><div><span className="eyebrow">SPECIAL DAYS</span><h2>值得期待的日子，不要错过</h2><p>商家优惠统一收在一张卡片中。</p></div><button onClick={()=>setShowSpecial(true)}>＋ 添加特别日子</button></div><div className="special-grid compact-specials"><button className="promo-folder" onClick={()=>setShowPromos(true)}><i>％</i><div><span>每周 · 每月 · 每年自动重复</span><h3>商家优惠活动</h3><p>{data.specialDays.filter(x=>x.id.startsWith("promo-")).length} 项优惠 · 点击展开完整清单</p></div><b>查看全部 →</b></button>{data.specialDays.filter(x=>!x.id.startsWith("promo-")).sort((a,b)=>nextSpecialDate(a).localeCompare(nextSpecialDate(b))).map(x=>{const next=nextSpecialDate(x);return <article key={x.id}><i>{x.kind==="生日"?"🎂":x.kind==="演唱会"?"♫":x.kind==="折扣"?"％":"✦"}</i><div><span>{specialRuleLabel(x)}</span><h3>{x.title}</h3><p>{displayDate(next)} · 提前 {x.reminderDays} 天提醒</p></div><button onClick={()=>setDeleteTarget({kind:"special",id:x.id})}>×</button></article>})}</div></section>}
    {showPromos&&<Modal title="商家优惠活动" onClose={()=>setShowPromos(false)}><div className="promo-catalog">{[1,2,3,4,5].map(day=><details key={day} open={day===new Date().getDay()}><summary>每周{["日","一","二","三","四","五","六"][day]}<span>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="weekly"&&x.weekday===day).length} 项</span></summary><div>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="weekly"&&x.weekday===day).map(x=><p key={x.id}>％ {x.title}</p>)}</div></details>)}<details><summary>每月会员日<span>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="monthly").length} 项</span></summary><div>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="monthly").sort((a,b)=>(a.monthDay||0)-(b.monthDay||0)).map(x=><p key={x.id}><b>每月 {x.monthDay} 日</b> · {x.title}</p>)}</div></details><details><summary>生日与年度权益<span>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="yearly").length} 项</span></summary><div>{data.specialDays.filter(x=>x.id.startsWith("promo-")&&x.recurrence==="yearly").sort((a,b)=>(a.monthDay||0)-(b.monthDay||0)).map(x=><p key={x.id}><b>{x.month} 月 {x.monthDay} 日</b> · {x.title}</p>)}</div></details></div></Modal>}
